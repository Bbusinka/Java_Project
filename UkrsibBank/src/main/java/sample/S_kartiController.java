...
public class S_kartiController {
...
//повернення на вікно системи
        back.setOnAction(event -> {
            ...
            stage.show(); });
//підтвердження зняття коштів з карти
            contin.setOnAction(event -> {  if (!suma.getText().isEmpty() && Integer.parseInt(suma.getText())>0){
                double a=user1.getSumm();
                double b=a-Integer.parseInt(suma.getText());
                user1.setSumm(b);
                //-----------------------------
                Druk druk = new Druk(user1.getSurname(),user1.getName(),user1.getPatronymic(),Integer.parseInt(suma.getText()),0, "4073"+" **** "+"**** "+user1.getN_karti(), Double.parseDouble(String.valueOf(new DecimalFormat("#0.00").format(b)).replace(',','.')));
                //-----------------------------
